...
<br>
`data` is just an arbitrary **variable** name to hold the result of `read.table` and it can be called/named *almost* anything.
...
We conducted `r n` statistical tests. The computed *p*-values need to be corrected for *multiple testing*. The correction can be performed using `p.adjust`, which simply takes the orignial *p*-values a vector and returns the adjusted (corrected) *p*-values:
